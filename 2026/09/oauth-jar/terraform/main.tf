...
resource "keycloak_openid_client" "jar" {
  for_each = local.jar_clients

  name                                       = "myclient-${each.key}"
  client_id                                  = "${var.myrealm_client_id}-${each.key}"
  realm_id                                   = keycloak_realm.myrealm.id
  ...
  client_secret_wo                           = "${var.myrealm_client_secret}-${each.key}"
  ...
  // https://github.com/keycloak/keycloak/blob/26.5.5/server-spi-private/src/main/java/org/keycloak/protocol/oidc/OIDCConfigAttributes.java
  ...
  valid_redirect_uris = [
    "${var.myrealm_base_url}/callback/${each.key}",
