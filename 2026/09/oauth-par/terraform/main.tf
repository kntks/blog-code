resource "keycloak_realm" "myrealm" {
  ...
  sso_session_idle_timeout_remember_me = "5h0m0s"
  sso_session_max_lifespan             = "10h0m0s"
  sso_session_max_lifespan_remember_me = "10m0s"
...
resource "keycloak_openid_client" "main" {
  name                                       = "myclient"
  client_id                                  = var.myrealm_client_id
  realm_id                                   = keycloak_realm.myrealm.id
  ...
  client_secret_wo                           = var.myrealm_client_secret
  ...
  // https://github.com/keycloak/keycloak/blob/26.7.3/server-spi-private/src/main/java/org/keycloak/protocol/oidc/OIDCConfigAttributes.java
  extra_config = {
  ...
  valid_redirect_uris = [
    "${var.myrealm_base_url}/callback"
  ]
  web_origins = []
}

resource "keycloak_openid_client" "binding" {
  name                                       = "myclient-binding"
  client_id                                  = "${var.myrealm_client_id}-binding"
  realm_id                                   = keycloak_realm.myrealm.id
  client_authenticator_type                  = "client-secret"
  access_token_lifespan                      = null
  access_type                                = "CONFIDENTIAL"
  ...
  client_offline_session_idle_timeout        = null
  client_offline_session_max_lifespan        = null
  client_secret_wo                           = "${var.myrealm_client_secret}-binding"
  client_secret_wo_version                   = 1
  client_session_idle_timeout                = null
  client_session_max_lifespan                = null
  consent_required                           = false
